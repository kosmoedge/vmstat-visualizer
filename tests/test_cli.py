...
class TestVisualizeCommand:
    ...
    @patch("vmstat_visualizer.plotly_renderer.render_html")
    @patch(
        "vmstat_visualizer.parser.parser.check_vmstat_columns",
        return_value=(True, True),
    )
    def test_visualize_html_calls_render_html(
        self,
        mock_check,
        mock_render_html,
        active_log,
        ...
        runner = CliRunner()
        prefix = str(tmp_path / "htmlviz")
        mock_render_html.return_value = str(tmp_path / "dummy_out.html")
        result = runner.invoke(
            cli,
            ["visualize", active_log, "-o", prefix, "-e", "html"],
        )
        assert result.exit_code == 0
        mock_render_html.assert_called_once()
        assert mock_render_html.call_args.args[0].filename == active_log
        assert mock_render_html.call_args.kwargs.get(
            "output_file_prefix",
        ) == prefix
